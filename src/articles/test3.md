...
## カテゴリ機能
Markdown上部に`category`を入力する場所がありますが、そこに`src/_data/site.json`で設定したslugを入力することで、記事にカテゴリを設定することができます。`src/_includes/article.njk`で表示される各記事に表示される他、そこからのリンクで同じカテゴリに属する他の記事を閲覧できる上、[カテゴリ一覧ページ](/allcategory)から全てのカテゴリを閲覧することも可能です。